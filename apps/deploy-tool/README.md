...
页面入口:`provisioner.html`(向导:第 4 步嵌入 `EditorApp`,第 5 步页面发布 + 规则发布 + `.scadaproj` 下载,T3.7)、`editor.html`(独立组态编辑器,自带 TB 连接面板,联调用)、`site.html`(大屏,T3.8 改为带登录的独立薄壳)。早期演示首页 `index.html` / `App.vue` 已在迁入时删除。
