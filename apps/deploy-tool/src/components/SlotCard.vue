...
const chartSeries = computed(() => {
  ...
  if (sl.card === 'combo') {
    const out = [
      { name: m(sl.device, sl.key).label || sl.key, color: srcColor(0), data: series(sl.device, sl.key), area: true },
    ]
    const r = (sl.extra || [])[0]
    if (r)
      out.push({ name: m(r.device, r.key).label || r.key, color: srcColor(1), data: series(r.device, r.key), y2: true })
    return out
  }
  return [
    {
...
<template>
  <div class="card" :class="{ alarming: sl.device && isAlarming(sl.device, sl.key) }">
    <div class="card-head">
      <span class="card-title"
        >{{ sl.title
          ...
          sl.kind === 'report'
            ? sl.deviceLabel || '自然日报表'
            : sl.card === 'alarmlist'
              ? '实时告警'
              : sl.card === 'multi'
                ? `${1 + (sl.extra?.length || 0)} 序列`
                : sl.device
        }}</span></span
      >
      <div v-if="sl.kind === 'report'" class="card-side">
        <div class="side-kv">
          <div class="k">{{ rep?.mode === 'month' ? '最近月净收益' : '最近日净收益' }}</div>
          <div class="v">{{ fmt(rep?.latestNet, 2) }} 元</div>
        </div>
      </div>
      <div v-else-if="sl.card === 'alarmlist'" class="card-side">
        <div class="side-kv">
          <div class="k">活动告警</div>
          <div class="v" :style="{ color: alarms.length ? 'var(--bad)' : 'var(--ok)' }">{{ alarms.length }}</div>
        </div>
      </div>
      <div v-else-if="isChart || sl.card === 'gauge'" class="card-side">
        <div class="side-kv">
          <div class="k">当前</div>
          <div class="v">{{ fmt(latest(sl.device, sl.key)) }} {{ meta(sl.device, sl.key).unit }}</div>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="sl.card === 'gauge'" class="card-body gauge-body" :class="{ compact }">
      <GaugeArc
        :value="latest(sl.device, sl.key)"
        :max="sl.max || 100"
        :unit="meta(sl.device, sl.key).unit"
        :color="color"
      />
    ...
    <!-- 地图 -->
    <VehicleMap
      v-else-if="sl.card === 'map'"
      :lat="latest(sl.device, 'latitude')"
      :lng="latest(sl.device, 'longitude')"
    />
