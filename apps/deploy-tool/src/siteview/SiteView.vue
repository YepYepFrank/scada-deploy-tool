...
import { login, getDevices, getHistory, getToken, API_BASE, PUBLIC_ID, kzRevenueTrend, reportAuth, getCustomerDevices } from '../api/tb'
...
async function loadReports() {
  ...
  for (const sid of [...new Set(slots.map((s) => s.device))]) {
    ...
    } catch (e) { console.warn('报表加载失败', sid, e) }
  }
}
...
const items = computed(() =>
  (cfg.value?.display || []).filter((i) => i.card !== 'none' && i.kind !== 'alarm'))
...
const layoutStats = computed(() =>
  (layoutT.value?.stats || []).map((id) => layoutSlots.value[id]).filter(Boolean))
...
async function addEntities(ents, tok) {
  ...
  await Promise.all(
    ents.map(async (ent) => {
      ...
      for (const i of items)
        (byWin[historyMinutes(i.key, i.card)] ||= []).push(i.key)
  ...
  connect(ents, tok)
  ents.forEach((e) => { if (e.entityType === 'DEVICE') { devIdByName[e.name] = e.id.id; devTokByName[e.name] = e.tok } })
}
...
function allDisplaySlots() {
  ...
  for (const i of items.value)
    if (i.card !== 'map' && i.kind !== 'report') out.push({ device: i.device, key: i.key })
  return out
}
function scanGaps() {
  const seen = new Set()
...
async function viewLogin() {
  if (!authPanel.user.trim() || !authPanel.pass) { authPanel.msg = '请输入账号密码'; return }
  authPanel.busy = true
  authPanel.msg = ''
  try {
    ...
    if (!found.length && authNeed.value.length) { authPanel.msg = '该账号也看不到这些设备,请换有权限的账号'; return }
    viewTok.value = tok
    await addEntities(found, tok)
    authNeed.value = authNeed.value.filter((n) => !found.some((f) => f.name === n))
    try {
      if (authPanel.remember)
        localStorage.setItem(LS_VIEW, btoa(unescape(encodeURIComponent(
          JSON.stringify({ u: authPanel.user.trim(), p: authPanel.pass, ts: Date.now() })))))
    } catch { /* 存储不可用时静默 */ }
    authPanel.open = authNeed.value.length > 0
    if (!authPanel.open) authPanel.pass = ''
    setTimeout(scanGaps, 6000) // 登录补数后复查缺口
  } catch (e) { authPanel.msg = String(e.message || e) } finally { authPanel.busy = false }
}
/* 静默重放已保存的登录(30 天) */
async function tryAutoViewLogin() {
  try {
    ...
    if (Date.now() - s.ts > 30 * 86400000) { localStorage.removeItem(LS_VIEW); return }
    authPanel.user = s.u
    authPanel.pass = s.p
    await viewLogin()
  } catch { /* 自动登录失败则显示登录条 */ }
...
onMounted(async () => {
  try {
    ...
    } catch (e) { console.warn('报表账号设备补充失败', e) }
    for (const c of (cfg.value.computations || []).filter((x) => ['aggregate.crossEntity', 'revenue.periodic'].includes(x.template))) {
    ...
    await addEntities(entities.filter((e) => !e.tok), null)
    await addEntities(entities.filter((e) => e.tok), repTok)
...
provide('slotCtx', {
  latest, series, meta, fmt, reportData, activeAlarms, isAlarming,
  palette: PALETTE, sinceText,
})
</script>
  ...
  <div class="shell">
    ...
    <header class="topbar">
      ...
      <div class="brand">
        ...
        <span class="brand-sub">{{ hdr.subtitle || (cfg?.site?.label !== pageTitle && cfg?.site?.label) || 'Config-Driven' }}</span>
      </div>
      <div class="topbar-right">
        <span class="live-pill" :class="store.status"><span class="live-dot"></span>{{ store.status.toUpperCase() }}</span>
    ...
    <div v-else-if="state === 'error'" class="boot"><span class="err">{{ errMsg }}</span></div>
    ...
    <template v-else>
      ...
      <section v-if="authNeed.length || (dataGaps.length && !viewTok)" class="auth-note">
        <template v-if="!authPanel.open">
          <span class="an-text">🔐
            <template v-if="authNeed.length">{{ authNeed.length }} 台设备需要登录后才能显示数据({{ authNeed.slice(0, 3).join('、') }}{{ authNeed.length > 3 ? '…' : '' }})</template>
            <template v-else>{{ dataGaps.length }} 个指标位暂无数据({{ dataGaps.slice(0, 3).map((s) => s.device + '·' + s.key).join('、') }}{{ dataGaps.length > 3 ? '…' : '' }})——若属权限受限设备,登录后可显示</template>
          </span>
        ...
        <template v-else>
          ...
          <input v-model="authPanel.pass" class="an-input" type="password" placeholder="密码" @keyup.enter="viewLogin" />
          <label class="an-rem"><input type="checkbox" v-model="authPanel.remember" />记住 30 天</label>
          <button class="an-btn" :disabled="authPanel.busy" @click="viewLogin">{{ authPanel.busy ? '登录中…' : '登录' }}</button>
      ...
      <section v-else-if="dataGaps.length && viewTok" class="auth-note quiet">
        <span class="an-text">ℹ️ 已登录,仍有 {{ dataGaps.length }} 个指标位无数据({{ dataGaps.slice(0, 3).map((s) => s.device + '·' + s.key).join('、') }}{{ dataGaps.length > 3 ? '…' : '' }})——测点可能未上报或键名有变</span>
      ...
      <section v-if="bannerAlarms.length" class="alarm-banner">
        <div v-for="a in bannerAlarms" :key="a.name" class="alarm-row" :class="a.severity.toLowerCase()">
          ...
          <span class="al-sev">{{ { CRITICAL: '严重', WARNING: '警告', MINOR: '提示' }[a.severity] || a.severity }}</span>
          <span class="al-msg"><b>{{ a.name }}</b> — {{ a.message }}</span>
      ...
      <nav v-if="pagesList.length > 1" class="page-nav">
        <button v-for="(p, i) in pagesList" :key="p.id" class="pn-item" :class="{ on: curPageIdx === i }"
                @click="curPageIdx = i">{{ p.title }}</button>
      </nav>

      <!-- 组态布局渲染 -->
      <template v-if="hasLayout">
        <section v-if="layoutStats.length" class="stat-rail"
          :style="{ gridTemplateColumns: `repeat(${Math.min(layoutStats.length, 6)}, 1fr)` }">
          <template v-for="(s, i) in layoutStats" :key="'ls' + i">
            <div v-if="s.card === 'gauge'" class="stat stat-gauge"
                 :class="{ alarming: isAlarming(s.device, s.key) }"
                 :style="{ '--stat-color': colorOf(i), animationDelay: i * 60 + 'ms' }">
              <div class="stat-label"><span>{{ s.title }}</span><span>GAUGE</span></div>
              <GaugeArc :value="latest(s.device, s.key)" :max="s.max || 100"
                        :unit="meta(s.device, s.key).unit" :color="colorOf(i)" />
              <div class="stat-sub">{{ s.device }} · {{ s.key }}</div>
            </div>
            <StatTile v-else-if="s.kind !== 'alarm'"
              :class="{ alarming: isAlarming(s.device, s.key) }"
              :label="s.title" :en="s.device.replace('Sim ', '')"
              :value="latest(s.device, s.key)" :unit="meta(s.device, s.key).unit"
              :color="colorOf(i)" :delay="i * 60" :sub="`${s.device} · ${s.key}`" />
            <div v-else class="stat" :class="{ alarming: !!alarmEntryByName(s.key) }"
                 :style="{ '--stat-color': alarmEntryByName(s.key) ? 'var(--bad)' : 'var(--ok)',
                           animationDelay: i * 60 + 'ms' }">
              <div class="stat-label"><span>{{ s.title }}</span><span>ALARM</span></div>
        ...
        <section v-if="tripleCols" class="triple-grid">
          <div class="tcol">
            <SlotCard v-for="(g, i) in tripleCols.left" :key="g.id" :sl="g.slot" compact
                      :color="colorOf(i)" :style="{ animationDelay: i * 70 + 'ms' }" />
          </div>
          <div class="tcol tcol-center">
            <div class="card main-reserve">
              <div class="mr-inner"><span class="mr-icon">🗺</span><span>{{ tripleCols.reservedLabel }}</span></div>
            </div>
            <SlotCard v-for="(g, i) in tripleCols.center" :key="g.id" :sl="g.slot"
                      :color="colorOf(3 + i)" :style="{ animationDelay: (3 + i) * 70 + 'ms' }" />
          </div>
          <div class="tcol">
            <SlotCard v-for="(g, i) in tripleCols.right" :key="g.id" :sl="g.slot" compact
                      :color="colorOf(5 + i)" :style="{ animationDelay: (5 + i) * 70 + 'ms' }" />
          </div>
        </section>
        <section v-else class="grid">
          <SlotCard v-for="(g, i) in layoutGrid" :key="g.id" :sl="g.slot"
                    :color="colorOf(layoutStats.length + i)"
                    :style="{ animationDelay: 120 + i * 60 + 'ms', gridColumn: `span ${g.span}` }" />
        </section>
      </template>

      <section v-if="!hasLayout && (statItems.length || alarmBadges.length)" class="stat-rail"
        :style="{ gridTemplateColumns: `repeat(${Math.min(statItems.length + alarmBadges.length, 6)}, 1fr)` }">
        <StatTile v-for="(it, i) in statItems" :key="it.device + it.key"
          :class="{ alarming: isAlarming(it.device, it.key) }"
          :label="meta(it.device, it.key).label" :en="it.device.replace('Sim ', '')"
          :value="latest(it.device, it.key)" :unit="meta(it.device, it.key).unit"
          :color="colorOf(i)" :delay="i * 60" :sub="it.device" />
        <div v-for="(b, i) in alarmBadges" :key="'al' + b.name" class="stat"
             :class="{ alarming: !!b.activeEntry }"
             :style="{ '--stat-color': b.activeEntry ? 'var(--bad)' : 'var(--ok)',
                       animationDelay: (statItems.length + i) * 60 + 'ms' }">
          <div class="stat-label"><span>{{ b.name }}</span><span>ALARM</span></div>
      ...
      <section v-if="!hasLayout" class="grid">
        <div v-for="(it, i) in chartItems" :key="it.device + it.key" class="card"
             :class="{ alarming: isAlarming(it.device, it.key) }"
             :style="{ animationDelay: 120 + i * 60 + 'ms' }">
          <div class="card-head">
            <span class="card-title">{{ meta(it.device, it.key).label }}<span class="en">{{ it.device }}</span></span>
            <div class="card-side">
              <div class="side-kv"><div class="k">当前</div>
                <div class="v">{{ fmt(latest(it.device, it.key)) }} {{ meta(it.device, it.key).unit }}</div></div>
            </div>
          </div>
          <div class="card-body">
            <LineChart :unit="meta(it.device, it.key).unit" :series="[{
        ...
        <div v-for="dev in mapDevices" :key="dev" class="card" style="animation-delay: 300ms">
          <div class="card-head"><span class="card-title">位置<span class="en">{{ dev }}</span></span></div>
