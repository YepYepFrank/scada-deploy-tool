...
## 3.2 执行与复验(2026-09-07 晚)

YY 选 A。**完成于 2026-09-07 晚**:并到「仙人山服务区」,39 资产 + 7 台 `BS_*` 设备已改分(记录与回退表 `联调记录/归属迁移-2026-09-07.md`);客户账号 `xrs-viewer` 复验「运营总览」8 个绑定实体全部 200、单文件大屏无「不可见 / 不可用」(截图 `img/归属迁移-客户视角-2026-09-07.jpg`)。 客户视角现在与租户视角一致:9/9 有数(其中两条 kz 曲线在 kz 鉴权修好后仍应可见——实体已归同一客户)。T4.2 只剩宿主应用侧(同事)与生产同款账号的走查。
## 4. 顺带发现
