...
- 声明里不再需要的种类,把对应的链删掉;删告警链之前先用 `unwireRootChain` 摘掉 Root 上本站点自己的转发节点(只动自己的,别人的节点与连线原样);摘完 Root 上还有别人的节点转发给它的,只清空不删(2026-09-11)。
...
- 结果拼进对应步骤的结果行,例如 `alarm:ok 无(已删上一版的 Site Alarms · S)`;摘了 Root 上本站点节点的带「已摘除 Root 上本站点的转发节点」,Root 上还有别人的节点转发着的写成「已清空未删:Root 上还有别人的节点转发到它」。
