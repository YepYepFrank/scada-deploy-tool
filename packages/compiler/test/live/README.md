...
| `conflict.live.ts` | 写入指纹与冲突检测 + 整体取绝对值(2026-09-11):临时站点(`Test Device A1 / A2`)发布「`abs(A1 − A2)` 存结果资产」与「A1 上 5 分钟周期统计」→ 同步两项都「一致」,TB 读回的表达式是 `abs(a - b)`、算出来 ≥ 0 → 模拟同事在 TB 里去掉 abs、把统计链定时周期 300 改成 600 → 同步两项都判「冲突」,差异精确到「表达式」与「节点 · periodInSeconds」→ 冲突选「待定」再发布(`skip`):TB 上两处都没被覆盖、再同步仍是冲突 → 再发布(以向导为准)回到一致。`afterAll` 兜底 cleanup 并删 A1 上临时结果 key。不建告警,不碰 Root 链 | 可跑 |
...
注意:用例的临时站点跑完后 CF 已删,但设备上会留下 `t25xxxx_*` 的遥测 key(历史点 7 天 TTL 后清),绑定的数据源面板里可能看到,对功能无影响。
