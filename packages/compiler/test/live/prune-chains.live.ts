// live(R1,2026-09-08):声明里删掉运算之后再发布,对应的站点规则链要真的从 TB 上消失,
// Root 上的转发节点也要摘掉。用独立命名的临时站点跑,不碰 xrs-mirror-test;afterAll 兜底清理。
//
...
describe.skipIf(!hasCreds)(`发布时清理旧链(live @ ${TB_BASE})`, () => {
  ...
  const rootFlowNames = async () => {
    ...
    return ((meta?.nodes ?? []) as { name: string }[]).map(n => n.name)
  }
  ...
  it('先发布带告警 + 多级归档的临时站点:两条链与 Root 转发都在', async () => {
    ...
    expect(await rootFlowNames()).toContain(chainNames.rootFlow(SITE))
  }, 120000)

  it('声明里去掉全部运算再发布:两条链从 TB 上消失,Root 转发被摘,且别人的链一条没少', async () => {
    ...
    expect(await rootFlowNames()).not.toContain(chainNames.rootFlow(SITE))
    ...
    expect(log.find(l => l.startsWith('alarm:ok'))).toContain('已摘除 Root 转发')
