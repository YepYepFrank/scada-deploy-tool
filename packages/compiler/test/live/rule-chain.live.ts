...
describe.skipIf(!hasCreds)(`规则链路径(live @ ${TB_BASE})`, () => {
  ...
  it('publish:CF 数量、规则链名 / 节点数、Root 上本站点的转发节点、站点资产与计划一致', async () => {
    const plan = compile(cfg, { devices: devIds })
  ...
  it('cleanup:临时站点的 CF / 链 / Root 上本站点的转发节点 / 资产全部清掉;xrs-mirror-test 前后零差异', async () => {
