...
内测第二轮第 8 条「遥测数据最好是小数点在一个纵轴上」,样子参照现有组态里的数字仪表。`SldDoc` 只加可选字段,`v` 不变;
**但数值标签的缺省画法变了**(见第一条),升级后页面上的数值会变样。

- **数码框(缺省)**:`sld` 组件新增属性 `valueStyle`,缺省 `'meter'`——数值标签画成**前缀 + 黑底框 + 单位**,
  框里是七段数码管数字、**右对齐**、每位等宽,小数点点在前一位右下角不占位;没亮的段淡淡垫一层「8」。
  数码管是自己画的 SVG path,不依赖字体包。**位数相同的数值小数点落在同一条竖线上**(「400.0 / 125.6 / 32.1」)。
  没值亮「--」;枚举文字(「停止」)写在框里;过期变灰、出错变红与原来一致。
  颜色可在祖先上覆盖 `--sr-sld-meter-bg` / `--sr-sld-meter-line` / `--sr-sld-meter-ink`。
  要回到纯文字「Uab 388.7 V」:组件属性 `valueStyle: 'plain'`,或单个标签 `look: 'plain'`。
- **叠在一起的自动对成一列**:x 相同、上下相邻(行距不超过 2.5 倍字号)的一串数码框,取同一个列宽——
  「P / SOC / 状态」这种前缀长短不一的一组,框的右边界自动对齐,不用再点编辑器的「对齐成列」。配了 `colW` 的以 `colW` 为准。
- **新字段**:`SldLabel`(数值)的 `look?: 'meter' | 'plain'`(不设随组件)、`cells?: number`(数码框位数,1–12,缺省 4;
  值更长时框向左加宽,右边界不动)。`colW` 在数码框下就是框的右边界。
- **读数可访问**:数码框整组 `role="img"`,`aria-label` 是完整读数(「Uab 388.7 V」),`data-value` 是数字文字。
- **导出**:`meterLayout` / `meterNaturalColW` / `autoMeterColumns` / `meterBoxWidth` / `meterCells` / `sevenSegPaths`
  与 `METER_CELLS_*`(部署工具的画布画同样的框、「对齐成列」算列宽用)。

## 0.11.0 — 2026-09-23

内测第二轮(《新开发一次图功能测试 20260923》)前六条里渲染器的部分。`SldDoc` 仍只加可选字段,`v` 不变;
**但开关的缺省画法变了**(见第一条),升级后页面上的开关会变样。

- **开关状态色(缺省)**:`sld` 组件新增属性 `switchStyle`,缺省 `'state'`——**合闸红色、分闸绿色、通信异常灰色**。
  断路器、手车、负荷开关、简化开关画成实心方块(三态同形,只靠颜色分,图元定义新增 `stateBlock`);
  隔离刀、接地刀保留刀闸形状,只把动触头上色。状态色块**不跟着失电变暗**(合就是红、分就是绿,和上游有没有电无关),
  失电变暗 / 不确定变虚只落在图形其余部分。颜色可在祖先上覆盖 `--sr-sld-sw-closed / -open / -unknown`。
  要回到国标图形 + 带电着色:组件属性 `switchStyle: 'classic'`。`<SldSymbol>` / `<SldSymbolBox>` 的 `switchStyle` 缺省仍是 classic(图元面板按形状认图元)。
- **开关位置不再判过期**:`staleSeconds` 只管数值变灰。开关只在变位时上报,一周没变是常态——现场 `T2_CB = 1` 停在 09-15,
  以前被当成过期画成虚线,合、分看起来一样,下游母线也跟着变「不确定」。现在「通信异常(灰)」只在两种情况下出现:
  **设备在线灯(active)明确离线**,或**测点从没收到过值**;在线灯没数据不算离线。配了 `state.fallback` 的仍按 fallback。
- **端口压在母线上就算连通**:图元端口坐标正好落在母线线段上,连通图里多一条 `attach` 边(恒通),带电色传得过去,
  不用再画连线。与 0.7.0 的「母线搭母线」同一个思路。`onBus()` 导出。
- **设备框线宽 / 虚线**:`SldNode.lineWidth`(缺省 2)、`SldNode.dashed`,只对 `freeBody` 图元(设备框)有效。
  设了其中之一、没改宽高时也按参数重画(线宽不会被倍数放大);虚线段跟着线宽拉长。`freeBody(w, h, style)` 多了第三个参数。
